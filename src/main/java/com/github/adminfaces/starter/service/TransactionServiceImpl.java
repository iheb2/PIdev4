...
import com.github.adminfaces.starter.model.TypeT;
...
/////////////////////////////////payer en agence//////////////////////////////////////		
		...
		@Override
		public void paycreditDirect(Long id_c) {
			List<Transaction> list = (List<Transaction>) trsansactionRepository.findAll();
		...
		@Override
		public void paycreditPortaPort(Long id_c) {
			List<Transaction> list = (List<Transaction>) trsansactionRepository.findAll();
			List<Transaction> list1 = new ArrayList<Transaction>();
			long diff;
			for (Transaction t : list) {
...
//payer une tranche du crdit.
	...
	@Override
	public List<Transaction> Historique(long id_c) {
		List<Transaction> list =(List<Transaction>) trsansactionRepository.findAll();
		List<Transaction> list1 =new ArrayList<Transaction>();
		for(Transaction i:list) {if(i.getAc().getId()==id_c&&i.getState()==StateT.oui )
			{list1.add(i);}} 
...
{   AmalWacelGhada e = aw.findById(Long.parseLong(id)).orElse(null);
		if(a<=e.getSomme()){
			...
			T.setNb(0);
			T.setAc(e);
			e.setSomme(e.getSomme()-a);
			T.setRetard((long) 0);
			T.setTotal(0);
			T.setMode(mode.A);
			T.setArgent(e.getSomme()-a);
			T.setState(StateT.oui);
