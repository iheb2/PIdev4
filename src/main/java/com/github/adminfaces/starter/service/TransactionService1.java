 ...
 */
@Component
public class TransactionService1 implements Serializable {
	 @Autowired
    ITransactionService it;
    ...
    private List<Predicate<Transaction>> configFilter(Filter<Transaction> filter) {
     ...
     /*   if (filter.hasParam("minPrice") && filter.hasParam("maxPrice")) {
            Predicate<Transaction> minMaxPricePredicate = (Car c) -> c.getPrice()
            ...
            Predicate<Transaction> minPricePredicate = (Car c) -> c.getPrice()
                    >= Double.valueOf((String) filter.getParam("minPrice"));
            predicates.add(minPricePredicate);
        } else if (filter.hasParam("maxPrice")) {
            Predicate<Transaction> maxPricePredicate = (Car c) -> c.getPrice()
                    <= Double.valueOf((String) filter.getParam("maxPrice"));
        ...
        if (has(filter.getEntity())) {
            ...
            if (has(filterEntity.getType())) {
                Predicate<Transaction> namePredicate = (Transaction c) -> c.getType().toString().toLowerCase().contains(filterEntity.getType().toString().toLowerCase());
    ...
    public List<StateT> getModels(String query) {
        return allCars.stream().filter(c -> c.getState().toString()
                .toLowerCase().contains(query.toLowerCase()))
                .map(Transaction::getState)
                .collect(Collectors.toList());
    }
    public void  direct(Long id_c){it.paycreditDirect(id_c);}
    public void retaita (String m,double a){it.retrait(m,a);}
    ...
    public void validate(Transaction car) {
        ...
        if (allCars.stream().filter(c -> c.getType().toString().equalsIgnoreCase(car.getType().toString())
